## my_cvm cvm_function Oct23_2018_TS_cvm_fun Jan26_TS_new_cvm_fun3
## 1. cvm function redefine (from cvm.test)

#' This is not your mother's Cramer-von Mises goodness of fit function!
#'
#' Calculate a goodness of fit metric given something which I do not yet understand.
#'
#' @param x the something I do not yet understand.
#' @param null The null distribution we are comparing against.
#' @param ... Extra arguments passed to intersect()
#' @param nullname name of the null hypothesis to be printed upon completion.
#' @return List containing the result of the test.
#' @export
my_cvm <- function(x, null = "punif", ..., nullname) {
  ...
  omega2 <- 1/(12 * n) + sum(fBasics::Heaviside(k, 0.25 * n) *
                             (U - (2 * k - 1) / (2 * n)) ^ 2)
  ## -- changed to 0.5
  ...
  if (length(parnames) > 0) {
    ...
    pard <- character(0)
    for (i in seq_along(parnames)) pard[i] <- paste(parnames[i], "=",
