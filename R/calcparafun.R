## calcparafun 2. calculate parameters

#' Calculate the parameters!  (I don't yet know what they are, but I suspect
#' they are the mu and sigma in the text of the paper.
#'
#' @param strain Bacterial strain to examine.
#' @param usable_tally_list Presumably the parsed tally.txt
#' @param save_location Where to save the result of this function.
#' @param rep_time How many times to repeat the parameter collection, I presume.
#' @return List containing the strain/experiment specific parameters.
...
####################### Calculating parameters #######################
  ...
  repgene_list <- lapply(filtered_list, function(x) {
    x <- x %>%
      ...
      dplyr::summarise(Nta = n())
  })
  consistent_genes <- Reduce(function(x, y) {
    dplyr::inner_join(x, y, by = c("Locus.CIA", "Nta")) }, repgene_list) ## 5708 genes
  filtered_list <- lapply(filtered_list, function(x) {
    x <- x %>%
      dplyr::filter(Locus.CIA %in% unique(consistent_genes$Locus.CIA))
  })
  ...
  parameter_list <- lapply(filtered_list, function(x) {
    ...
    totdata <- totdata %>%
      dplyr::group_by(Nta) %>%
      dplyr::mutate(ngene = n()) %>% ungroup()
    nsample <- oridata %>%
      dplyr::group_by(Locus.CIA) %>%
      dplyr::sample_n(1) %>%
      dplyr::select(Locus.CIA, n)
    totdata <- totdata %>%
      dplyr::left_join(nsample, by = "Locus.CIA")
    ...
    ntareslist <- lapply(ntalist, function(y) {
      ...
      y <- unlist(optim(
        c(0.9, log(mean(dplyr::filter(tot, Nta == nta)$gtot) / nta), 1),
        function(pars) {
          my_cvm(samdata, "jointfun", pars[1], pars[2], pars[3], nta)$statistic
        }))
      print(y)
    })
  ...
  saveRDS(parameter_list, paste(dirname(save_location),
